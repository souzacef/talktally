...
const expense: TransactionResponse = {
  ...
  installmentCount: 1,
  occurrences: [
    { sequenceNumber: 1, effectiveDate: '2026-08-19', amount: '30.06', currency: 'BRL' },
  ],
}
...
describe('TransactionDetailPage', () => {
  ...
  it('fetches the route transaction ID and shows a loading state while it resolves', async () => {
    ...
    mocks.get.mockImplementation(() => new Promise<TransactionResponse>((resolve) => {
  ...
  it.each<[TransactionSource, string]>([
    ...
    ['VOICE', 'Voice'],
  ])('shows %s with the friendly %s source label', async (source, label) => {
  ...
  it('renders the authoritative multi-installment response without regenerating values', async () => {
    ...
    expect(within(card!).getByText('Oct 12, 2026')).toBeInTheDocument()
